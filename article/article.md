...
Сначала мы строим сумки слов -- это базовые описатели документов. Вообще говоря, это разреженные вектора огромных размерностей(десятки и сотни тысяч, по количеству слов в словаре). С векторами такой размерности работать не удобно. Поэтому мы используем методы понижения размерностей. Тематическое моделирование -- это метод понижения размерности с дополнительными свойствами интерпретируемости.

Далее по корпусу документов мы строим матрицы, которые помогают нам переходить от многомерного сумочного векторного представления к компактному векторному представлению в "тематическом" пространстве(размерность: десятки или сотни). Сам алгоритм работает так, чтобы это компактное представление "максимально хорошо" описывало наблюдаемый корпус документов. В вероятностных тематических моделях "максимально хорошо" -- это функционал максимума правдоподобия(что уже не обязательно знать для понимания статьи).

Итак, на выходе: у нас есть матрицы, которые позволяют переходить в компактное тематическое пространство из огромного сумко-словного пространства. Эти же матрицы позволяют сделать обратный переход(с потерей информации).


### Ссылки на подробности
[Лекции](https://youtu.be/H7hlSz4WWhQ?t=12m29s)(временная метка на начале техподробностей) Константина Воронцова по этой теме. 3 часа лекций и вы в теме.
...
### Что есть что: PLSA, LDA, HDP, ARTM, в чем разница?
Если вы пользователь "классических" методов, зачем вам ARTM? 
